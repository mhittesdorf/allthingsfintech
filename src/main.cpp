...
namespace {
    ...
    BOOST_AUTO_TEST_CASE(testNoShortSales) {
        ...
        //constraints implemented as C++ 11 lambda expressions
        noShortSalesConstraints[0] = [] (const Array & x){Real x4 = 1.0 - (x[0] + x[1] + x[2]);
            return (x[0] >= 0.0 && x[1] >= 0.0 && x[2] >= 0.0 && x4 >= 0.0);};
        noShortSalesConstraints[1] = [] (const Array & x){Real x4 = 1.0 - (x[0] + x[1] + x[2]);
    ...
    BOOST_AUTO_TEST_CASE(testNoShortSalesWithPositionLimits) {
        ...
        //constraints implemented as C++ 11 lambda expressions
        noShortSalesConstraints[0] = [] (const Array & x){Real x4 = 1.0 - (x[0] + x[1] + x[2]);
            return (x[0] >= 0.05 && x[1] >= 0.05 && x[2] > 0.05 && x4 > 0.05);};
        noShortSalesConstraints[1] = [] (const Array & x){Real x4 = 1.0 - (x[0] + x[1] + x[2]);
            return 1.0 - (x[0] + x[1] + x[2] + x4) < 1e-9;};
        noShortSalesConstraints[2] = [] (const Array & x){Real x4 = 1.0 - (x[0] + x[1] + x[2]);
            return (x[0] <= .50 && x[1] <= .50 && x[2] <= .50 && x4 <= .50);};
